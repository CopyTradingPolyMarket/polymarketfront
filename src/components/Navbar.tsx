...
interface Category { label: string; href: string; }
      ...
      {focused && (
        <div className="absolute top-full mt-3 left-0 w-full z-[9999] rounded-2xl bg-[#121316] backdrop-blur-xl overflow-hidden shadow-[0_30px_100px_rgba(0,0,0,.65)]">
          <div className="px-4 py-3 border-b border-white/[0.05] flex items-center justify-between">
            <p className="text-[11px] text-gray-500 uppercase tracking-wider">{query ? "Results" : "Trending Markets"}</p>
            <span className="text-[10px] text-gray-600">{results.length} items</span>
          </div>
          <div className="max-h-[420px] overflow-y-auto pr-1">
            {results.map((market) => (
              <Link key={market.id} href={`/markets/${slugify(market.title)}`}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-medium text-white truncate group-hover:text-indigo-200 transition-colors">{market.title}</p>
            ...
            {results.length === 0 && <div className="py-10 text-center text-sm text-gray-500">No markets found</div>}
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <nav className="border-b border-[#1e1f23] px-4">
      <ul className="flex items-center gap-0 overflow-x-auto scrollbar-none">
        <li>
          <button onClick={() => setActive("Trending")} className={`cursor-pointer flex items-center gap-1.5 px-3 py-2.5 text-sm whitespace-nowrap transition-colors duration-150 border-b-2 ${active === "Trending" ? "border-white text-white" : "border-transparent text-gray-400 hover:text-gray-200"}`}>
            <TrendingIcon /><span className="font-medium">Trending</span>
          </button>
        </li>
        <li>
          <button onClick={() => setActive("Breaking")} className={`cursor-pointer px-3 py-2.5 text-sm whitespace-nowrap transition-colors duration-150 border-b-2 ${active === "Breaking" ? "border-white text-white" : "border-transparent text-gray-400 hover:text-gray-200"}`}>Breaking</button>
        </li>
        <li className="h-4 w-px bg-[#2a2b2f] mx-1" aria-hidden />
        <li>
          <button onClick={() => setActive("New")} className={`cursor-pointer px-3 py-2.5 text-sm whitespace-nowrap transition-colors duration-150 border-b-2 ${active === "New" ? "border-white text-white" : "border-transparent text-gray-400 hover:text-gray-200"}`}>New</button>
        </li>
        <li className="h-4 w-px bg-[#2a2b2f] mx-1" aria-hidden />
        {CATEGORIES.slice(0, 15).map((cat) => (
          <li key={cat.label}>
            <button onClick={() => setActive(cat.label)} className={`px-3 py-2.5 text-sm whitespace-nowrap transition-colors duration-150 border-b-2 cursor-pointer ${active === cat.label ? "border-white text-white" : "border-transparent text-gray-400 hover:text-gray-200"}`}>
              {cat.label}
            </button>
          </li>
        ))}
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {authenticated ? (
              ...
              <>
                ...
                <button
                  onClick={openSignup}
                  className="px-3 py-1.5 text-[13px] font-medium text-white bg-[#34d399] hover:bg-[#3ee6aa] transition-colors rounded-lg cursor-pointer"
                >
                  Sign Up
                </button>
              </>
            ...
            <button
              ...
              className="p-1.5 rounded-lg hover:bg-[#1a1b1e] transition-colors text-gray-400"
            >
              <HamburgerIcon />
            </button>
          </div>
      ...
      <SideDrawer
        ...
        onOpenAuth={(mode) => { setDrawerOpen(false); setTimeout(() => { setAuthMode(mode); setAuthOpen(true); }, 180); }}
      />
