...
function MobileBetSheet({
  ...
  const onTouchStart = (e: React.TouchEvent) => {
    ...
    startH.current = sheetRef.current?.getBoundingClientRect().height ?? 0;
  };
  ...
  return (
    <>
      ...
      {/* Spacer so content isn't hidden behind sheet */}
...
export default function MarketPage() {
  ...
  useEffect(() => {
    const found = MOCK_MARKETS.find((m: Market) => slugify(m.title) === slug);
  ...
  const sharedPanelProps = { market, activeOption, setActiveOption, betType, setBetType, amount, setAmount, estimatedShares, potentialProfit, prob, router };
    ...
    <div style={{ minHeight: "100vh", background: "#09090b", color: "#fff", fontFamily: "'DM Sans','Helvetica Neue',sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 16 : 20 }}>
          ...
          {/* Option pills — horizontal scroll on mobile */}
          ...
          <div style={{ background: "#0f0f12", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 20, padding: isMobile ? "16px 14px 10px" : "20px 20px 12px" }}>
            ...
            <ResponsiveContainer width="100%" height={isMobile ? 160 : 220}>
              <AreaChart data={chartData} margin={{ top: 4, right: 0, left: -28, bottom: 0 }}>
                ...
                <XAxis dataKey="date" tick={{ fill: "#4b5563", fontSize: 9 }} tickLine={false} axisLine={false} interval={Math.floor(chartData.length / (isMobile ? 3 : 4))} />
          ...
          {/* Related markets — shown below on mobile */}
          {isMobile && (
